...
- `.env.example` documenting required variables:
  - `GEMINI_API_KEY`, `GEMINI_TEXT_MODEL` (e.g. `gemini-flash-latest`), `GEMINI_IMAGE_MODEL` (e.g. `gemini-3.1-flash-image`), `GEMINI_PRO_IMAGE_MODEL` (e.g. `gemini-3-pro-image`), `SUPABASE_URL`, `SUPABASE_KEY` (Service Role), `PORT=8000`, `NODE_ENV=production`.
